 ...
 * @module @dsh-local/dsh-skin-switcher
 */
...
const SELF_PACKAGE = '@dsh-local/dsh-skin-switcher'
...
/**
 * Enumerate installed skin packages under the profile node_modules. Every
 * entry whose directory name matches dsh-client-ui-skin-* (in any scope,
 * e.g. `@dsh-external/dsh-client-ui-skin-maid-atelier`) and that carries a
 * valid skin.json is a skin home. The switcher's own package and any skin
 * manager package are excluded.
...
export function listSkinDirCandidates(modulesDir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.startsWith('@')) {
      // Scoped dir: its children are the packages.
      let scopedEntries
      try {
      ...
      for (const pkg of scopedEntries) {
        ...
        if (!statSync(pkgPath, { throwIfNoEntry: false })?.isDirectory()) continue
        if (!SKIN_PACKAGE_RE.test(pkg)) continue
        if (`@${entry}/${pkg}` === SELF_PACKAGE) continue
        if (statSync(join(pkgPath, 'skin.json'), { throwIfNoEntry: false })) out.push(pkgPath)
      }
    } else {
      if (!SKIN_PACKAGE_RE.test(entry)) continue
      if (entry === SELF_PACKAGE) continue
      if (statSync(join(entryPath, 'skin.json'), { throwIfNoEntry: false })) out.push(entryPath)
    }
  }
  return out
}
 ...
 * with live facts: a skin is bundle-wired when either its skin.json says so
 * or its package is listed in the profile's dsh.profile.bundles (the
 * `dsh plugin add` path). Entry dirs are stored for the state payload.
...
export function loadRegistry(modulesDir, profileManifestPath) {
  ...
  for (const dir of listSkinDirCandidates(modulesDir)) {
    ...
    out[meta.id] = {
      ...
      bundleWired: meta.bundleWired || bundles.has(meta.package),
      dir,
      name: meta.name,
      nameEn: meta.nameEn,
      author: meta.author,
