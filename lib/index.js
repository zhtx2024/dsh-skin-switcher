...
/**
 * Which cordis row ids currently wire the skin-center manager, discovered
 * from the bundle patches of the profile's bundle packages. The aggregate
 * renames its rows over time (`ui-skin-center` → `web-ui-skin-center` in
 * dsh-web-ui 0.1.20), so the switcher must not hardcode the id: it disables
 * every composed row whose `name` resolves to the installed
 * `dsh-client-ui-skin-center` package.
 *
 * Defensive fallback: when the center package is installed but no bundle
 * row naming it could be found, the legacy id `ui-skin-center` is returned
 * so the manager is still disabled under the old convention.
 * @param modulesDir - profile node_modules dir.
 * @param profileManifestPath - profile package.json path.
...
export function findSkinCenterRowIds(modulesDir, profileManifestPath) {
  ...
  for (const bundle of bundles) {
    ...
    for (const m of text.matchAll(/- id: ([a-z0-9-]+)\s*\r?\n\s*name:\s*['"]?([^'"\s]+)/g)) {
      if (m[2].replace(/^@[^/]+\//, '') === centerBare) ids.add(m[1])
    }
  }
  if (ids.size === 0 && centerInstalled) ids.add('ui-skin-center')
  return [...ids]
}
 ...
 * @param centerRowIds - composed skin-center row ids to disable
 *   (see findSkinCenterRowIds).
 */
export function renderManaged(active, registry, centerRowIds = []) {
  const body = []
  ...
  // disabled so it can never write a competing managed section. Its row id
  // changes across aggregate versions, so it is discovered from the bundle
  // patches instead of being hardcoded.
...
export function useSkin(target, registry) {
  ...
  const next = `${patch.length > 0 ? `${patch}\n\n` : ''}${renderManaged(official ? null : target, registry, centerRowIds)}\n`
